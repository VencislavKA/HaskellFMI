...
main = do
    ...
    print $ averageBalanceOfCities (accounts1, people1) ["Burgas"] == 24.95 -- my test
...
averageBalanceOfCities database cities = roundTwoDigButWithMagic $ sum filteredBalance / fromIntegral (length filteredBalance)
 where
...
getAverageBalance database p = roundTwoDigButWithMagic $ sum filteredBalance / fromIntegral (length filteredBalance)
 where
    accounts = fst database
    persons = snd database
